      ...
      (() => {
        ...
        async function toDataUrl(file, targetW) {
          ...
          try {
            ...
            const scale = Math.min(
              1,
              cap / Math.max(bitmap.width, bitmap.height)
            );
        ...
        class ImageSlot extends HTMLElement {
          ...
          constructor() {
            ...
            this._spill.addEventListener("pointerdown", (e) => {
              ...
              const fw = rect.width || 1,
                fh = rect.height || 1;
              const corner =
                e.target.getAttribute && e.target.getAttribute("data-c");
              let move;
              if (corner) {
          ...
          _clampView() {
            ...
            const mx = Math.max(
              0,
              ((g.iw * g.base * this._view.s) / g.fw - 1) * 50
            );
            const my = Math.max(
              0,
              ((g.ih * g.base * this._view.s) / g.fh - 1) * 50
            );
          ...
          _render() {
            ...
            const shape = (
              this.getAttribute("shape") || "rounded"
            ).toLowerCase();
            let radius = "";
