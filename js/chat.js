...
Rules: Be warm, concise and helpful (2-4 sentences max). You cannot book jobs or give exact prices, but you can explain services and always encourage calling 022 074 6441 or using the quote form for a free quote. If the customer wants to be contacted, book a job, or get a callback, tell them to tap the "Request a callback" button below the chat (or call 022 074 6441). Only answer about this business and home signal/TV/wifi/security topics.`;
        ...
        function fallbackReply(q) {
          ...
          if (/(price|cost|quote|how much|charge)/.test(s))
            return (
              "We tailor every job, so prices vary — but quotes are free and there's no obligation." +
              call
            );
          ...
          if (/(wifi|wi-fi|internet|network|modem)/.test(s))
            return (
              "We set up and optimise whole-home WiFi, networking and 4G modems for fast, stable coverage." +
              call
            );
          if (/(signal|reception|pixel|tuning|tune)/.test(s))
            return (
              "Poor reception, pixelation and tuning faults are our bread and butter — we diagnose on-site and optimise your signal." +
              call
            );
          if (/(cctv|camera|security|smart)/.test(s))
            return (
              "We install CCTV and security cameras and set up smart-home devices, with full tech support." +
              call
            );
          if (/(area|cover|auckland|where|location|suburb)/.test(s))
            return "We cover all of Auckland — North Shore, West, Central, East, Manukau, Papakura, Franklin, Hibiscus Coast and Waiheke Island.";
          if (/(senior|discount|elderly|pension)/.test(s))
            return (
              "Yes! We proudly offer discounted rates for senior citizens." +
              call
            );
          if (/(hi|hello|kia ora|hey)/.test(s))
            return "Kia ora! How can I help with your TV, signal or WiFi today?";
          return (
            "Happy to help with TV aerials, satellite, reception, WiFi, CCTV or IT support." +
            call
          );
        }
