...
(function () {
  ...
  let W = 0, H = 0, DPR = 1;
  let nodes = [], pulses = [], rings = [], waves = [], txi = [], floaters = [], shooters = [];
  let satSprite = null, rocketSprite = null;
  let raf = null, last = 0;
  ...
  function makeSatelliteSprite() {
    ...
    g.beginPath();
    g.moveTo(-15, -5); g.lineTo(-15, 5);
    g.moveTo(15, -5); g.lineTo(15, 5);
    g.moveTo(-21, 0); g.lineTo(-9, 0);
    g.moveTo(9, 0); g.lineTo(21, 0);
    g.stroke();
    // dish + mast
    g.beginPath(); g.moveTo(0, -7); g.lineTo(0, -11); g.stroke();
    g.beginPath(); g.arc(0, -13, 4.5, Math.PI * 0.95, Math.PI * 2.05); g.stroke();
    // core glow
  ...
  function makeRocketSprite() {
    ...
    g.lineTo(6, 11); g.lineTo(-6, 11); g.lineTo(-6, 7);
    g.quadraticCurveTo(-7, -7, 0, -19);
    g.stroke();
    // window
    g.beginPath(); g.arc(0, -4, 3, 0, Math.PI * 2); g.stroke();
    // fins
    g.beginPath();
    g.moveTo(-6, 5); g.lineTo(-12, 13); g.lineTo(-6, 11);
    g.moveTo(6, 5); g.lineTo(12, 13); g.lineTo(6, 11);
    g.stroke();
    // exhaust flame
    g.strokeStyle = 'rgba(130,205,255,0.9)';
    g.beginPath(); g.moveTo(-3.2, 11); g.lineTo(0, 19); g.lineTo(3.2, 11); g.stroke();
    return c;
  }
  ...
  function size() {
    ...
    for (let i = 0; i < txCount; i++) txi.push((Math.random() * count) | 0);

    // radio-wave traces
    waves = [
      { y: H * 0.34, amp: 16, len: 240, sp: 0.018, ph: 0, op: 0.10 },
      { y: H * 0.72, amp: 22, len: 320, sp: 0.012, ph: 2, op: 0.08 },
    ];

    // floating satellites + rockets
    const big = Math.min(W, H) > 620;
    const kinds = big ? ['sat', 'rocket', 'sat', 'rocket'] : ['sat', 'rocket'];
    floaters = kinds.map((type) => {
      if (type === 'rocket') {
        ...
        return { type, x: Math.random() * W, y: Math.random() * H,
          ...
          scale: 0.55 + Math.random() * 0.4, bob: Math.random() * Math.PI * 2, wob: Math.random() * Math.PI * 2 };
      }
      return { type, x: Math.random() * W, y: Math.random() * H,
        vx: (Math.random() - 0.5) * 0.07, vy: (Math.random() - 0.5) * 0.07,
        rot: Math.random() * Math.PI * 2, vr: (Math.random() - 0.5) * 0.0007,
        scale: 0.6 + Math.random() * 0.5, bob: Math.random() * Math.PI * 2 };
    });

    rings.length = 0; pulses.length = 0;
    start();
  }
  ...
  function spawnPulse() {
    ...
    if (best) pulses.push({ a, b: best, t: 0, sp: 0.012 + Math.random() * 0.014 });
  }
  function spawnRing() {
    // emit from the satellite floater so rings track its live position
    const sat = floaters.find(f => f.type === 'sat');
    if (sat) {
      rings.push({ ref: sat, x: sat.x, y: sat.y, r: 4, max: 180 + Math.random() * 120, life: 1 });
    } else if (txi.length) {
      const n = nodes[txi[(Math.random() * txi.length) | 0]];
      if (n) rings.push({ x: n.x, y: n.y, r: 4, max: 150 + Math.random() * 120, life: 1 });
    }
  }
  function spawnShooter() {
    const fromLeft = Math.random() < 0.5;
    const x = fromLeft ? -60 + Math.random() * W * 0.35 : W * 0.65 + Math.random() * W * 0.4;
    const y = -60 + Math.random() * H * 0.25;
    const a = (fromLeft ? 0.20 : 0.80) * Math.PI + (Math.random() - 0.5) * 0.18;
    const speed = 7 + Math.random() * 4;
    shooters.push({
      x, y,
      vx: Math.cos(a) * speed, vy: Math.abs(Math.sin(a)) * speed,
      len: 90 + Math.random() * 90,
      width: 1.1 + Math.random() * 0.9,
      life: 1, fade: 0.0006 + Math.random() * 0.0004,
    });
  }

  let pulseTimer = 0, ringTimer = 0, shooterTimer = 0, shooterNext = 2200;
  function frame(now) {
    ...
    // radio-wave traces
    if (!reduce) for (const wv of waves) {
      wv.ph += wv.sp * dt * 0.06;
      ctx.beginPath();
      for (let x = -40; x <= W + 40; x += 8) {
        const y = wv.y + Math.sin(x / wv.len * Math.PI * 2 + wv.ph) * wv.amp
    ...
    if (!reduce) for (const n of nodes) {
      ...
      if (d2 < 26000) { n.x += dx * 0.0008 * dt; n.y += dy * 0.0008 * dt; }
    }
    ...
    ringTimer += dt;
    if (ringTimer > 1100 && !reduce) { ringTimer = 0; spawnRing(); }
    ...
    for (const r of rings) {
      ...
      ctx.beginPath(); ctx.arc(rx, ry, r.r, 0, Math.PI * 2);
    ...
    for (const f of floaters) {
      if (!reduce) {
        ...
        f.bob += 0.0013 * dt;
        if (f.type === 'sat') f.rot += f.vr * dt; else f.wob += 0.0016 * dt;
      }
      if (f.x < -margin) f.x = W + margin; if (f.x > W + margin) f.x = -margin;
      if (f.y < -margin) f.y = H + margin; if (f.y > H + margin) f.y = -margin;
      ...
      else angle = Math.atan2(f.vy, f.vx) + Math.PI / 2 + Math.sin(f.wob) * 0.12;
      ...
      ctx.drawImage(sp, -sp.width * s / 2, -sp.height * s / 2, sp.width * s, sp.height * s);
      ctx.globalAlpha = 1;
      ctx.restore();
    }
    ...
    if (shooterTimer > shooterNext && !reduce) {
      shooterTimer = 0; shooterNext = 2600 + Math.random() * 4200;
      spawnShooter();
    }
    shooters = shooters.filter(s => s.life > 0 && s.x > -200 && s.x < W + 200 && s.y < H + 160);
    for (const s of shooters) {
      if (!reduce) { s.x += s.vx * dt * 0.09; s.y += s.vy * dt * 0.09; s.life -= s.fade * dt; }
      ...
      ctx.fillStyle = `rgba(238,250,255,${0.95 * s.life})`; ctx.fill();
    }
  ...
  function onScroll() {
    ...
    const o = Math.max(0, Math.min(1, (window.scrollY - vh * 0.45) / (vh * 0.5)));
