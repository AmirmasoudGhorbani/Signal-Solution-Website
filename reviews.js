...
(function () {
  ...
  function fitHeight() {
    let max = 0;
    cards.forEach((c) => {
      const vis = c.style.visibility, pos = c.style.position, op = c.style.opacity;
      c.style.visibility = 'hidden'; c.style.position = 'relative'; c.style.opacity = '0';
      max = Math.max(max, c.offsetHeight);
      c.style.visibility = vis; c.style.position = pos; c.style.opacity = op;
    });
    stage.style.minHeight = max + 'px';
  }
  fitHeight();
  let fitT; window.addEventListener('resize', () => { clearTimeout(fitT); fitT = setTimeout(fitHeight, 180); });
