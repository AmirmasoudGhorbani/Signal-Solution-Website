...
(function () {
  ...
  let W = 0, H = 0, DPR = 1;
  let cx = 0, cy = 0;          // Auckland hub (px)
  const CX = 0.455, CY = 0.46; // normalized hub position on the image
  let rings = [];
  let pulses = [];
  let twinkles = [];
  let sweep = 0;
  ...
  function spawnRing() { rings.push({ r: 6, life: 1 }); }
  function spawnPulse() {
    const ang = Math.random() * Math.PI * 2;
    const dist = Math.min(W, H) * (0.28 + Math.random() * 0.5);
    pulses.push({ ang, t: 0, dist, speed: 0.006 + Math.random() * 0.006 });
  }
  function spawnTwinkle() {
    twinkles.push({
      x: Math.random() * W, y: Math.random() * H,
      t: 0, life: 0.5 + Math.random() * 0.6, r: 1 + Math.random() * 1.8,
    });
  }

  let ringTimer = 0, pulseTimer = 0, twinkTimer = 0;
  ...
  function frame(now) {
    ...
    // timers
    ringTimer += dt; pulseTimer += dt; twinkTimer += dt;
    if (ringTimer > 1500) { ringTimer = 0; spawnRing(); }
    if (pulseTimer > 520)  { pulseTimer = 0; spawnPulse(); }
    if (twinkTimer > 140)  { twinkTimer = 0; spawnTwinkle(); }
    ...
    pulses.forEach(p => {
      ...
      const ty = cy + Math.sin(p.ang) * p.dist * Math.max(0, e - 0.16) * 0.62;
    ...
    twinkles.forEach(t => {
      ...
      ctx.fillStyle = `rgba(120,200,255,${a})`; ctx.fill();
    });
  ...
  const io = new IntersectionObserver((es) => {
    es.forEach(e => { visible = e.isIntersecting; if (visible) start(); else stop(); });
  }, { threshold: 0.05 });
