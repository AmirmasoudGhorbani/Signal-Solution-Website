...
(function () {
  ...
  const IMAGES = [
    { src: A.dish,      crop: [0.20, 0.04, 0.74, 0.90], lum: 70, bb: 76 },
    { src: A.satellite, crop: [0.06, 0.10, 0.88, 0.80], lum: 70, bb: 76 },
    // coverage: lower thresholds = denser map; exclude the bottom-left legend box
    { src: A.coverage,  crop: [0.05, 0.04, 0.90, 0.90], lum: 52, bb: 56, gain: 1.25,
  ...
  const easeInOut = (t) => { t = t < 0 ? 0 : t > 1 ? 1 : t; return t < 0.5 ? 4 * t * t * t : 1 - Math.pow(-2 * t + 2, 3) / 2; };
  ...
  function sampleImage(img, spec) {
    const crop = spec.crop, lumT = spec.lum, bbT = spec.bb;
  ...
  function initParticles() {
    ...
    for (let p = 0; p < N; p++) {
      ...
      particles[p] = {
        sx, sy,            // morph source (home)
        tx, ty,            // morph target (home)
        hx: sx, hy: sy,    // current interpolated home
        rx: 0, ry: 0, rvx: 0, rvy: 0,   // cursor-ripple displacement
        x: sx, y: sy,
        b: cloud[p * 3 + 2],
  ...
  function refitTargets(toIndex) {
    ...
    for (let p = 0; p < N; p++) {
      const pt = particles[p];
      const [tx, ty] = targetXY(p, cloud);
      pt.tx = tx; pt.ty = ty; pt.sx = tx; pt.sy = ty; pt.hx = tx; pt.hy = ty;
      pt.b = cloud[p * 3 + 2];
    }
  }
  ...
  function frame(now) {
    ...
    if (intro) {
      mp = elapsed / INTRO;
      interpolating = true;
      if (mp >= 1) { intro = false; mp = 1; phase = 0; phaseStart = now; }
    } else if (!morphing && elapsed > HOLD) {
      morphing = true;
      phaseStart = now;
      setTargets((phase + 1) % clouds.length);
      mp = 0; interpolating = true;
    } else if (morphing) {
      mp = elapsed / MORPH;
      interpolating = true;
      if (mp >= 1) { morphing = false; phase = (phase + 1) % clouds.length; phaseStart = now; mp = 1; interpolating = false; }
    }
    ...
    const sW = scaleFor();
    const WAVE_PERIOD = 5000, WAVE_SWEEP = 2700;
    const wph = now % WAVE_PERIOD;
    let waveX = null;
    if (wph < WAVE_SWEEP) waveX = (oxW - sW * 0.66) + (sW * 1.32) * (wph / WAVE_SWEEP);
    ...
    for (let p = 0; p < N; p++) {
      ...
      if (mouse.active) {
        const ddx = (pt.hx + pt.rx) - mouse.x, ddy = (pt.hy + pt.ry) - mouse.y;
      ...
      pt.rvx += -pt.rx * 0.10;     // spring back home
      pt.rvy += -pt.ry * 0.10;
      pt.rvx *= 0.86; pt.rvy *= 0.86;
      pt.rx += pt.rvx * dt;
      pt.ry += pt.rvy * dt;
      ...
      if (waveX !== null) {
        ...
        if (dd < WAVE_W) { wb = 1 - dd / WAVE_W; wb *= wb; }
      }
      const size = (2.4 + pt.b * 7) * tw * (1 + wb * 1.5);
      ctx.globalAlpha = Math.min(1, (0.55 + pt.b * 0.45) * tw + wb * 0.55);
  ...
  function boot() {
    sprites = [
      makeSprite([[0,'rgba(232,248,255,1)'],[0.18,'rgba(150,225,255,0.95)'],[0.45,'rgba(70,180,255,0.55)'],[0.75,'rgba(34,150,255,0.18)'],[1,'rgba(20,120,255,0)']]),   // brand cyan
      makeSprite([[0,'rgba(255,255,255,1)'],[0.22,'rgba(223,243,255,0.92)'],[0.5,'rgba(159,212,255,0.45)'],[0.8,'rgba(120,180,255,0.14)'],[1,'rgba(120,180,255,0)']]), // ice white
      makeSprite([[0,'rgba(200,245,255,1)'],[0.2,'rgba(80,210,255,0.95)'],[0.5,'rgba(30,150,255,0.5)'],[0.8,'rgba(20,110,255,0.16)'],[1,'rgba(20,110,255,0)']]),     // azure
      makeSprite([[0,'rgba(224,230,255,1)'],[0.22,'rgba(150,170,255,0.9)'],[0.52,'rgba(95,110,240,0.45)'],[0.8,'rgba(70,90,220,0.14)'],[1,'rgba(70,90,220,0)']]),     // indigo hint
    ];
    spriteBright = makeSprite([[0,'rgba(255,255,255,1)'],[0.3,'rgba(234,246,255,0.95)'],[0.6,'rgba(180,228,255,0.5)'],[0.85,'rgba(120,200,255,0.16)'],[1,'rgba(120,200,255,0)']]);
    resize();
  ...
  document.addEventListener('visibilitychange', () => {
    ...
    else if (!document.hidden && ready && !raf) { t0 = performance.now(); raf = requestAnimationFrame(frame); }
  });
